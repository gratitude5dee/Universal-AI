    ...
    <div className="space-y-6">
      ...
      <Card className="glassmorphism p-6">
        ...
        <div className="space-y-8">
          {/* Pass Rate Chart */}
          <div>
            <h4 className="text-sm font-medium mb-4">Pass Rate Over Time</h4>
            <div className="h-48 rounded-lg bg-gradient-to-br from-accent/5 to-accent/10 border border-border/50 flex items-center justify-center">
              <p className="text-sm text-muted-foreground">Chart: Pass rate trending ↗ +3.2% this week</p>
            </div>
          </div>

          {/* Latency Chart */}
          <div>
            <h4 className="text-sm font-medium mb-4">Latency Distribution (p50, p95, p99)</h4>
            <div className="h-48 rounded-lg bg-gradient-to-br from-accent/5 to-accent/10 border border-border/50 flex items-center justify-center">
              <p className="text-sm text-muted-foreground">Chart: p50: 205ms | p95: 289ms | p99: 412ms</p>
            </div>
          </div>

          {/* Cost Chart */}
          <div>
            <h4 className="text-sm font-medium mb-4">Cost Per Eval Run</h4>
            <div className="h-48 rounded-lg bg-gradient-to-br from-accent/5 to-accent/10 border border-border/50 flex items-center justify-center">
              <p className="text-sm text-muted-foreground">Chart: Total This Week: $4.23 | Avg per Run: $0.12</p>
            </div>
          </div>
        </div>
      </Card>
      ...
      <Card className="glassmorphism p-6">
        ...
        <div className="space-y-6">
          <div>
            <p className="text-sm font-medium mb-2">This Month: $847.23</p>
            <div className="space-y-2">
              {[
                ...
                <div key={idx} className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium w-20 text-right">{item.amount}</span>
                  <span className="text-xs text-muted-foreground w-12">({item.percent}%)</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-border/30">
            <h4 className="text-sm font-semibold mb-3">💡 Optimization Recommendations</h4>
            <div className="space-y-3">
              {[
                ...
                <Card key={idx} className="p-4 bg-accent/5 border-border/30">
                  ...
                  <p className="text-xs text-muted-foreground mb-3">Impact: {rec.impact}</p>
      ...
      <Card className="glassmorphism p-6">
        <h3 className="text-lg font-semibold mb-4">🔍 Regression Detection</h3>
            ...
            <Card key={idx} className={`p-4 border ${
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="font-medium mb-1">{item.agent}</p>
